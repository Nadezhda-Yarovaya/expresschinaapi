  ...
  User.findById(id)
    .then((user) => {
      ...
      User.findByIdAndUpdate(
        ...
        {
          $set: {
            favsDaily: [...user.favsDaily, favoriteCard], //addtoset maybe used
          },
        },
        opts
      )
...
module.exports.removeFromFavs = (req, res, next) => {
  const opts = { new: true, runValidators: true };
  const { authorization } = req.headers;
  const keytoken = authorization.split(" ")[1];
  const id = encodeToken(keytoken)._id;
  ...
  const { favoriteCard } = req.body;
  console.log("current fav: ", favoriteCard);
  ...
  User.findOne({ email })
    ...
    .then((foundUser) => {
      ...
      bcrypt
        ...
        .then((matched) => {
          if (!matched) {
            return Promise.reject(new UnauthorizedError(wrongLoginDataMessage));
          }
          // create both tokens
